defmodule RelayHandler.PortPool do
  @moduledoc """
  Pool of UDP port pairs (RTP + RTCP) for the relay's media legs.

  ex_kamailio is a pure SDP shuttle and allocates no ports — picking the local
  media ports is the handler's job, so this pool lives in the example. Allocates
  even-numbered ports (the canonical RTP port; RTCP is the next odd port).
  Configured by the `:port_range` application env under `:relay_handler`.
  """
  ...
  @impl true
  def init(_opts) do
    range = Application.fetch_env!(:relay_handler, :port_range)
