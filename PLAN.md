...
Client A sends INVITE containing SDP to Kamailio.
Kamailio sends this SDP over websocket to ex_kamailio.
ex_kamailio forwards it to the developer’s custom code.
Developer’s custom code responses with SDP containing IP address of Membrane Endpoint that will be used to exchange multimedia with client B.
ex_kamailio passes this SDP to Kamailio, and Kamailio sends it in INVITE to client B.
Client B responds with OK, its SDP answer is sent via Kamailio and ex_kamailio to the developer’s custom code.
Now developer’s custom code passes to ex_kamailio IP address Membrane will use to exchange multimedia with client A.
ex_kamailio sends it to Kamailio.
Kamailio wraps it in an OK message and forwards it to client A.
